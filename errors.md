...
---
## [2026-08-12 Session 7] CRITICAL: users_admin_manage_center made every query on users fail with 42P17
#[security] #[rls] #[postgres]
**Context:** Re-reading schema.sql to write the per-role RLS harness that Step 8 has been waiting on.
**Error:** `create policy "users_admin_manage_center" on users for all using (role in ('teacher','admin') and center_id in (select center_id from users where id = auth.uid() and role = 'admin'));` — a policy **on** `users` that reads **from** `users`. While rewriting any statement that touches the table, Postgres expands the policy, reaches the subquery's reference to `users`, and re-applies the same policy to it; the rewriter's recursion guard aborts the statement with `42P17 infinite recursion detected in policy for relation "users"`. Not a slow query and not a wrong answer — a hard failure on every read. It would have taken the whole app down on first boot: `proxy.ts:81` looks the caller's role up in `users` on every authenticated request, and both `lib/utils/pageAuth.ts:28` and `lib/utils/apiAuth.ts:42` do the same per page and per API call. The lookup would have thrown, `role` would have resolved to `null`, and the Session 3 "role-less session is unauthenticated" branch would have bounced every real user — teacher, admin and parent alike — straight back to the login screen. The blast radius is wider than `users` itself: `students_all_teacher` and its siblings read from `users`, so expanding *their* policies re-enters `users`' policies and trips the same guard.
**Root cause:** The four admin-override policies were written as one block, all following the same `center_id in (select center_id from users where id = auth.uid() and role = 'admin')` shape. On `classes`, `sessions`, and `lessons` that shape is correct and recursion-free. Copied onto `users`, the identical expression becomes self-referential — the bug is in the one place the pattern could not be reused, and it reads as the most consistent line in the file. Static review kept passing over it because the logic is right; it is the *relation* that is wrong. Nothing caught it downstream either, since schema.sql has never been executed (Step 7 is still blocked on Gio's Supabase project), and a policy body is not parsed against the catalog until the statement runs.
**Fix:** Two `security definer` helpers, `public.current_user_role()` and `public.current_user_center()`, that read the caller's own row. `security definer` runs the body as the function owner, who is not subject to RLS, so no policy is expanded inside it and the loop is broken. `language plpgsql` rather than `sql` is deliberate: the planner inlines trivial `sql` functions, and an inlined body would sit back inside the caller's query tree with RLS applied again. (`set search_path` blocks inlining by itself — the language choice keeps that from being the only thing standing between the fix and a regression.) `users_admin_manage_center` now calls the helpers, each wrapped in `(select ...)` so the planner evaluates them once as an InitPlan rather than per row, and carries an explicit `WITH CHECK`. Execute is revoked from `public` and granted to `authenticated` **and `anon`** — policies are evaluated for whoever runs the query, so a logged-out request would otherwise get "permission denied for function" where the app expects an empty result. Same pass: `report_templates_select_staff` was `using (auth.uid() is not null)`, which is every authenticated user including parents — the name said staff, the expression said anyone. Now scoped to the three staff roles via the same helper; only the two staff report pages read that table.
**Prevention:** A policy on table T may never name T in its own body — reach for a `security definer` helper instead, and treat `language sql` as unsafe for one. When a policy shape is copied across tables, re-check it against the table it landed on, not against the table it came from: the admin block was four near-identical policies and only the one whose subject and object were the same table was wrong. And schema.sql is dead text until it runs — `supabase/rls-verification.sql` now exists so Step 7 is immediately followed by evidence rather than by assumption.
**Files affected:** schema.sql, supabase/rls-verification.sql (new)
---
